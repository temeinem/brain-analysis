function plotBlinkNumberPerSubject(session_ibi_len, good_ind)
    ...
    for j = 1:size(pair,2)
        ...
        for k = 1:size(pair{j}(:),1)
            %[s ind] = sort(session_ibi_len(pair{j}(k), good_ind(:)));
            %session_ibi_stat(good_ind(ind(:,1)), 1, pair{j}(k))
            plot(1:length(good_ind), session_ibi_len(pair{j}(k), good_ind(ind)),'LineStyle',':','Marker', markers{pair{j}(k)}, 'LineWidth', 2, 'MarkerSize',5, 'color', colors(pair{j}(k),:));
