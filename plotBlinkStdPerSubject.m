function plotBlinkStdPerSubject(session_ibi_stat, good_ind)
    ...
    for j = 1:size(pair,2)
        ...
        for k = 1:size(pair{j}(:),1)
            plot(good_ind, session_ibi_stat(good_ind, 2, pair{j}(k)),'LineStyle',':','Marker', markers{pair{j}(k)}, 'LineWidth', 2, 'MarkerSize',5, 'color', colors(pair{j}(k),:));
